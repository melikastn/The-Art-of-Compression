...
public:
    ...
    * Ties will be broken by taking the largest of the tied locations 
    * (rightmost, or bottommost, depending on splitting dimension)
    * 
...
private:
   ...
   */
   ...
   int minimizeSumVar(stats & s,pair<int,int> ul, pair<int,int> lr, bool vert);

   void renderHelper(PNG & img, Node* root);

   void pruneSizeHelper(int tol, Node * root, Node * subtree);

   int distance( Node* root , Node * curr);

   bool prunable(Node * root, Node * subtree, int tol);

   void prune(Node * root, Node * subtree, int tol);

   void clear(Node * root);

   Node * copy( Node * orig);

   void getEstimateTol(int leaves, int estTolL, int estTolU);
